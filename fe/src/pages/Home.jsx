...
export default function Home() {
  ...
  useEffect(() => {
    API.get("/flats/search").then(({ data }) => setFlatCount(data.length));
  }, []);

  if (isOwner) return <OwnerHome user={user} flatCount={flatCount} />;
  return <TenantHome user={user} flatCount={flatCount} />;
}

function OwnerHome({ user, flatCount }) {
  return (
    <div>
      {/* Owner Hero */}
      ...
      <section style={ownerStyles.statsBar}>
        {[
          { label: "Active Listings", value: flatCount || "0" },
          { label: "Cities Covered", value: "10+" },
          { label: "Verified Tenants", value: "500+" },
          { label: "Successful Rentals", value: "300+" },
        ].map((s, i) => (
      ...
      <section style={ownerStyles.featSection}>
        <div style={styles.inner}>
          <h2 style={styles.sectionTitle}>💼 Why List on FLATKART?</h2>
          <div style={styles.featGrid}>
            {ownerFeatures.map((f, i) => (
              <div key={i} style={ownerStyles.featCard}>
      ...
      <section style={styles.section}>
        <h2 style={styles.sectionTitle}>🚀 How It Works for Owners</h2>
        <div style={styles.stepsRow}>
          {ownerSteps.map((s, i) => (
            <div key={i} style={ownerStyles.stepCard}>
      ...
      {/* Testimonial / Trust */}
      <section style={ownerStyles.trustSection}>
        <div style={styles.inner}>
          <h2 style={{ ...styles.sectionTitle, color: "#fff" }}>🌟 Trusted by Property Owners Across India</h2>
          <div style={ownerStyles.trustGrid}>
            {[
              { quote: "Listed my flat and got a tenant within 3 days. Incredible platform!", name: "Ramesh K.", city: "Bhubaneswar" },
              { quote: "The dashboard makes managing bookings so easy. Highly recommended!", name: "Priya S.", city: "Cuttack" },
              { quote: "Finally a platform that gives owners full control over their listings.", name: "Amit D.", city: "Puri" },
            ].map((t, i) => (
              <div key={i} style={ownerStyles.trustCard}>
                <p style={ownerStyles.quote}>"{t.quote}"</p>
                <p style={ownerStyles.trustName}>— {t.name}, {t.city}</p>
              </div>
            ))}
          </div>
        </div>
...
function TenantHome({ user, flatCount }) {
  return (
    <div>
      {/* Tenant Hero */}
      ...
      <section style={styles.statsBar}>
        {[
          { label: "Flats Listed", value: flatCount || "0" },
          { label: "Cities Covered", value: "10+" },
          { label: "Happy Tenants", value: "500+" },
          { label: "Verified Owners", value: "200+" },
        ].map((s, i) => (
          <div key={i} style={styles.statItem}>
      ...
      <section style={styles.featSection}>
        <div style={styles.inner}>
          <h2 style={styles.sectionTitle}>✨ Why Choose FLATKART?</h2>
          <div style={styles.featGrid}>
