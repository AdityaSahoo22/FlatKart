...
export const register = async (req, res) => {
  ...
  if (role === "owner") {
    const owner = await Owner.create({ name, email, password: hashed, role: "owner" });
    res.json(owner);
  } else {
    const user = await User.create({ name, email, password: hashed, role: "tenant" });
    res.json(user);
  }
};
...
export const login = async (req, res) => {
  ...
  if (found && await bcrypt.compare(password, found.doc.password)) {
    const token = jwt.sign({ id: found.doc._id }, process.env.JWT_SECRET);
    res.json({ token, user: { id: found.doc._id, name: found.doc.name, email: found.doc.email, role: found.doc.role } });
  } else {
